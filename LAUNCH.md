...
`db push` aplicará solamente las migraciones pendientes, incluida
`007_customer_orders_and_support_chat.sql`. Esa migración habilita cancelación
segura de pedidos pendientes, ocultado del historial del cliente y chat privado
con políticas RLS. Si una ejecución anterior de `001` quedó a medias, la
migración puede reanudarse sin fallar por políticas ya existentes.
...
7. Enviar un mensaje desde Chat privado y responderlo desde Panel general > Chats.
